          ...
          <div className="hidden md:flex items-center gap-8 bg-root-card/80 backdrop-blur-md border border-white/10 rounded-lg px-8 py-3">
            {NAV_ITEMS.map((item) => (
              <Link
                key={item.label}
                to={item.href}
                className={`text-xs font-bold tracking-widest transition-colors uppercase ${location.pathname === item.href
                    ? 'text-root-accent'
          ...
          <div className="bg-root-card border border-white/10 rounded-xl p-6 flex flex-col gap-6 shadow-2xl backdrop-blur-xl">
            {NAV_ITEMS.map((item) => (
              <Link
                key={item.label}
                to={item.href}
                className={`text-sm font-bold tracking-widest border-b border-white/5 pb-4 last:border-0 ${location.pathname === item.href
                    ? 'text-root-accent'
                    : 'text-white hover:text-root-accent'
                  }`}
